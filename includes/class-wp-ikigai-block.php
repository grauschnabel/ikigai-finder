	...
	public static function handle_chat_request() {
		try {
			...
			if ( ! check_ajax_referer( 'wp_ikigai_chat', 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'Security check failed.', 'wp-ikigai' ) ), 403 );
				return;
			}
			...
			if ( empty( $api_key ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'API key not configured. Please configure the API key in the settings.', 'wp-ikigai' ) ), 400 );
				return;
			}
			...
			if ( empty( $system_prompt ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'System prompt not configured.', 'wp-ikigai' ) ), 400 );
				return;
			}
			...
			foreach ( $conversation as $message ) {
				if ( ! isset( $message['role'] ) || ! isset( $message['content'] ) ) {
					...
					wp_send_json_error( array( 'message' => __( 'Invalid message format in conversation.', 'wp-ikigai' ) ), 400 );
					return;
				}
				$messages[] = array(
